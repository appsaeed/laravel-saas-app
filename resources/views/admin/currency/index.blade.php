...
@section('title', __('locale.menu.Currencies'))
    ...
    <section id="datatables-basic">
        <div class="mb-3 mt-2">
            ...
            @can('create currencies')
                <div class="btn-group">
                    <a href="{{route('admin.currencies.create')}}" class="btn btn-success waves-light waves-effect fw-bold mx-1"> {{__('locale.buttons.add_new')}} <i data-feather="plus-circle"></i></a>
                </div>
            @endcan

            @can('manage currencies')
                <div class="btn-group">
                    <a href="{{route('admin.currencies.export')}}" class="btn btn-info waves-light waves-effect fw-bold"> {{__('locale.buttons.export')}} <i data-feather="file-text"></i></a>
                </div>
            @endcan
                ...
                <div class="card">
                    <table class="table datatables-basic">
                        ...
                        <tr>
                            ...
                            <th>{{__('locale.currencies.format')}}</th>
    ...
    <script>
        $(document).ready(function () {
            ...
            let dataListView = $('.datatables-basic').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('admin.currencies.search') }}",
                    "dataType": "json",
                    "type": "POST",
            ...
            Table.delegate(".get_status", "click", function () {
                ...
                $.ajax({
                    url: "{{ url(config('app.admin_path').'/currencies')}}" + '/' + currency_id + '/active',
                    type: "POST",
                    data: {
                        _token: "{{csrf_token()}}"
                    },
            ...
            Table.delegate(".action-delete", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/currencies')}}" + '/' + id,
                            type: "POST",
                            data: {
                                _method: 'DELETE',
                                _token: "{{csrf_token()}}"
            ...
            $(".bulk-enable").on('click', function (e) {
                ...
                Swal.fire({
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.currencies.enable_currencies')}}",
                    icon: 'warning',
                    showCancelButton: true,
                    ...
                    if (result.value) {
                        ...
                        $.each(rows_selected, function (index, rowId) {
                            currency_ids.push(rowId)
                        });

                        if (currency_ids.length > 0) {

                            $.ajax({
                                url: "{{ route('admin.currencies.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'enable',
            ...
            $(".bulk-disable").on('click', function (e) {
                ...
                Swal.fire({
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.currencies.disable_currencies')}}",
                    icon: 'warning',
                    showCancelButton: true,
