...
@section('title', __('locale.menu.Administrators'))
    ...
    <section id="datatables-basic">
        <div class="mb-3 mt-2">
            ...
            @can('create administrator')
                <div class="btn-group">
                    <a href="{{route('admin.administrators.create')}}" class="btn btn-success waves-light waves-effect fw-bold mx-1"> {{__('locale.buttons.create')}} <i data-feather="plus-circle"></i></a>
                </div>
            @endcan

            @can('view administrator')
                <div class="btn-group">
                    <a href="{{route('admin.administrators.export')}}" class="btn btn-info waves-light waves-effect fw-bold"> {{__('locale.buttons.export')}} <i data-feather="file-text"></i></a>
                </div>
            @endcan
    ...
    <script>
        $(document).ready(function () {
            ...
            let dataListView = $('.datatables-basic').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('admin.administrators.search') }}",
                    "dataType": "json",
                    "type": "POST",
            ...
            Table.delegate(".get_status", "click", function () {
                ...
                $.ajax({
                    url: "{{ url(config('app.admin_path').'/administrators')}}" + '/' + role_id + '/active',
                    type: "POST",
                    data: {
                        _token: "{{csrf_token()}}"
                    },
            ...
            Table.delegate(".action-delete", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/administrators')}}" + '/' + id,
                            type: "POST",
                            data: {
                                _method: 'DELETE',
                                _token: "{{csrf_token()}}"
            ...
            $(".bulk-enable").on('click', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        let administrator_ids = [];
                        let rows_selected = dataListView.column(1).checkboxes.selected();

                        $.each(rows_selected, function (index, rowId) {
                            administrator_ids.push(rowId)
                        });

                        if (administrator_ids.length > 0) {

                            $.ajax({
                                url: "{{ route('admin.administrators.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'enable',
                                    ids: administrator_ids
                                },
                                success: function (data) {
                                    showResponseMessage(data, dataListView);
                                },
