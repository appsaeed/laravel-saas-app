...
class EloquentAccountRepository extends EloquentBaseRepository implements AccountRepository {
    ...
    public function register( array $input ): User {
        ...
        if ( $customer ) {
            $customer->phone = isset( $input['phone'] ) ? $input['phone'] : null;
    ...
    public function redirectAfterLogin( Authenticatable $user ): Authenticatable {
        if ( config( 'app.two_factor' ) === false || $user->two_factor == 0 || Session::get( 'two-factor-login-success' ) == 'success' || $this->checks() ) {
